...
S = {
    ...
    activeTimeSlider=nil, draggingPlaylist=false, draggingPlaylistIndex=nil, draggingPlaylistTarget=nil, finishPlaylistDrag=nil,
...
function updatePlaylistDisplay()
    ...
    for idx,song in ipairs(S.playlistSongs) do
        ...
        dragHitbox.InputEnded:Connect(endPlaylistDrag)
...
UserInputService.InputChanged:Connect(function(input)
    if input.UserInputType==Enum.UserInputType.MouseMovement
        ...
        elseif S.draggingPlaylist and S.draggingPlaylistIndex then
            local frame=S.PlaylistScrollFrame
            if frame and frame.Parent then
                local draggingFrame=nil
                for _,child in ipairs(frame:GetChildren()) do
                    if child.Name=="PlaylistItem"
                        and child:GetAttribute("PlaylistIndex")==S.draggingPlaylistIndex then
                        draggingFrame=child
                        break
                    end
                end
                ...
                if draggingFrame then
                    ...
                    local y=(input.Position.Y-frame.AbsolutePosition.Y)+frame.CanvasPosition.Y
                    local maxY=math.max(0,frame.CanvasSize.Y.Offset-draggingFrame.AbsoluteSize.Y)
                    y=math.clamp(y-draggingFrame.AbsoluteSize.Y/2,0,maxY)
                    draggingFrame.Position=UDim2.new(0,3,0,y)

                    local itemStep=55+4
                    local target=math.clamp(math.floor((y+draggingFrame.AbsoluteSize.Y/2)/itemStep)+1,1,#S.playlistSongs)
                    if target~=S.draggingPlaylistIndex then
                        -- Animate the items around the dragged row.
                        local from=S.draggingPlaylistIndex
                        for _,child in ipairs(frame:GetChildren()) do
                            if child.Name=="PlaylistItem" then
                                local idx=child:GetAttribute("PlaylistIndex")
                                if idx then
                                    local newIndex=idx
                                    if from<target and idx>from and idx<=target then
                                        newIndex=idx-1
                                    elseif from>target and idx>=target and idx<from then
                                        newIndex=idx+1
                                    end
                                    if idx~=from then
                                        TweenService:Create(
...
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType==Enum.UserInputType.MouseButton1
        ...
        if S.draggingPlaylist then
            local activeIndex=S.draggingPlaylistIndex
            for _,child in ipairs(S.PlaylistScrollFrame and S.PlaylistScrollFrame:GetChildren() or {}) do
                if child.Name=="PlaylistItem" and child:GetAttribute("PlaylistIndex")==activeIndex then
                    local moved=child:GetAttribute("DragMoved")==true
                    local target=S.draggingPlaylistTarget or activeIndex
                    S.draggingPlaylist=false
                    S.draggingPlaylistIndex=nil
                    S.draggingPlaylistTarget=nil
                    if S.PlaylistScrollFrame then S.PlaylistScrollFrame.ScrollingEnabled=true end
                    if moved and target~=activeIndex then
                        reorderPlaylist(activeIndex,target)
                    end
                    break
                end
            end
...
local function createMainGUI()
    ...
    local infoTabs=Instance.new("Frame"); infoTabs.Size=UDim2.new(1,0,0,32); infoTabs.BackgroundColor3=Color3.fromRGB(40,45,50); infoTabs.BorderSizePixel=0; infoTabs.Parent=S.InfoPanel; infoTabs.ZIndex=11; Instance.new("UICorner",infoTabs).CornerRadius=UDim.new(0,6)
    S.InfoHowTab=Instance.new("TextButton"); S.InfoHowTab.Size=UDim2.new(0.5,0,1,0); S.InfoHowTab.Position=UDim2.new(0,0,0,0); S.InfoHowTab.BackgroundColor3=Color3.fromRGB(100,150,255); S.InfoHowTab.BorderSizePixel=0; S.InfoHowTab.Text="📖 HOW TO USE"; S.InfoHowTab.TextColor3=Color3.fromRGB(255,255,255); S.InfoHowTab.Font=Enum.Font.GothamBold; S.InfoHowTab.TextSize=11; S.InfoHowTab.Parent=infoTabs; S.InfoHowTab.ZIndex=12; Instance.new("UICorner",S.InfoHowTab).CornerRadius=UDim.new(0,6)
    S.InfoDataTab=Instance.new("TextButton"); S.InfoDataTab.Size=UDim2.new(0.5,0,1,0); S.InfoDataTab.Position=UDim2.new(0.5,0,0,0); S.InfoDataTab.BackgroundColor3=Color3.fromRGB(60,65,72); S.InfoDataTab.BorderSizePixel=0; S.InfoDataTab.Text="📊 DATA"; S.InfoDataTab.TextColor3=Color3.fromRGB(255,255,255); S.InfoDataTab.Font=Enum.Font.GothamBold; S.InfoDataTab.TextSize=11; S.InfoDataTab.Parent=infoTabs; S.InfoDataTab.ZIndex=12; Instance.new("UICorner",S.InfoDataTab).CornerRadius=UDim.new(0,6)
